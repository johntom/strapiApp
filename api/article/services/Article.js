...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Article
      ...
      .populate(_.keys(_.groupBy(_.reject(strapi.models.article.associations, {
        autoPopulate: false
      }), 'alias')).join(' '));
  },
  ...
  add: async (values) => {
    ...
    // console.log('services bakery data Article',data)
  ...
  edit: async (params, values) => {
    ...
    await strapi.hook.mongoose.manageRelations('article', _.merge(_.clone(params), {
      values
    }));
    return Article.update(params, values, {
      multi: true
    });
  },
  ...
  //   console.log('rm all article')
  //   const data = await Article.remove() // remove//(ctx.params);
