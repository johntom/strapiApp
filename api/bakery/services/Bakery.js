...
module.exports = {
  ...
  add: async (values) => {
    console.log('services bakery')
    const data = await Bakery.create(_.omit(values, _.keys(_.groupBy(strapi.models.bakery.associations, 'alias'))));
    //console.log('services bakery data',data)
    //await strapi.hook.mongoose.manageRelations('bakery', _.merge(_.clone(data), { values }));
